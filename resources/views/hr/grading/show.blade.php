...
<head>
    <meta charset="utf-8">
    <title>Grade Attempt — Crafting Colons</title>
...
<body class="min-h-full text-white py-12 px-4">
    <div class="max-w-2xl mx-auto space-y-6">
        <div>
            ...
            <p class="text-neutral-400 text-sm">{{ $attempt->assessment->title }}</p>
                ...
                <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6">
                    ...
                    @if ($question->type->usesOptions())
                        <ul class="text-sm space-y-1 mb-3">
                            @foreach ($question->options as $option)
                                ...
                                <li class="{{ $selected ? 'text-white font-medium' : 'text-neutral-500' }}">
                                    {{ $selected ? '☑' : '☐' }} {{ $option->label }}
                                    @if ($option->is_correct) <span class="text-emerald-400 text-xs">(correct)</span> @endif
                                </li>
                            @endforeach
                        </ul>
                        <p class="text-xs text-neutral-500">Auto-graded: {{ $answer->marks_awarded }} / {{ $question->marks }}</p>
                    ...
                    @else
                        @if ($question->type->value === 'file_upload' || $question->type->value === 'coding')
                            ...
                            @else
                                <p class="text-sm text-neutral-500 mb-3">No file submitted.</p>
                            @endif
                        @else
                            <p class="text-sm bg-neutral-800/60 rounded-lg p-3 mb-3 whitespace-pre-wrap">{{ $answer->text_answer ?: '(no answer submitted)' }}</p>
                        @endif

                        <div class="flex items-center gap-2">
                            <label class="text-sm text-neutral-300">Marks awarded:</label>
                            <input type="number" name="grades[{{ $loop->index }}][marks_awarded]"
                                ...
                                class="w-20 rounded-lg bg-neutral-800 border border-neutral-700 text-white px-3 py-1">
                            <span class="text-xs text-neutral-500">/ {{ $question->marks }}</span>
                        </div>
