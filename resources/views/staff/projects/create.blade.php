<x-layouts.app :title="'Articles — Crafting Colons'">
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <form method="POST" action="{{ route('staff.projects.store') }}" enctype="multipart/form-data"
            ...
            <textarea name="excerpt" placeholder="Short description / Excerpt" rows="2"
                class="w-full rounded-lg bg-ink-800 border border-ink-700 text-white px-3 py-2">{{ old('excerpt') }}</textarea>

            {{-- Body --}}
            <textarea name="body" placeholder="Detailed project case study body" rows="10" required
                class="w-full rounded-lg bg-ink-800 border border-ink-700 text-white px-3 py-2">{{ old('body') }}</textarea>

            {{-- Project Logistics --}}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm text-ink-300 mb-1">Client Name</label>
                    <input type="text" name="client_name" value="{{ old('client_name') }}" placeholder="e.g., Acme Corp" required
                        class="w-full rounded-lg bg-ink-800 border border-ink-700 text-white px-3 py-2">
                </div>
                <div>
                    <label class="block text-sm text-ink-300 mb-1">Project Type</label>
            ...
            {{-- Project URL --}}
            <div>
                <label class="block text-sm text-ink-300 mb-1">Project URL</label>
            ...
            <div>
                <label class="block text-sm text-ink-300 mb-1">Technologies Used</label>
                <select name="technologies[]" multiple class="w-full rounded-lg bg-ink-800 border border-ink-700 text-white px-3 py-2 min-h-[120px]">
                ...
                <p class="text-[11px] text-ink-500 mt-1">Hold Down Ctrl (Cmd on Mac) to select multiple items.</p>
            </div>

            {{-- Repeatable Project Results Section --}}
            <div class="border border-ink-800 rounded-xl p-4 bg-ink-950/40 space-y-3">
                <div class="flex items-center justify-between">
                    <label class="block text-sm font-medium text-ink-300">Project Key Results</label>
                    <button type="button" id="add-result-row" 
                        class="text-xs bg-ink-800 border border-ink-700 hover:bg-ink-700 px-2 py-1 rounded text-ink-200 transition">
                        + Add Metric Row
                    </button>
                </div>
                ...
                <div id="results-container" class="space-y-2">
                        ...
                        @foreach(old('results') as $index => $result)
                            <div class="grid grid-cols-12 gap-2 result-row">
                                <div class="col-span-7">
                                    <input type="text" name="results[{{ $index }}][metric]" value="{{ $result['metric'] ?? '' }}" placeholder="Metric Label (e.g. Conversion Rate Increase)" class="w-full rounded-lg bg-ink-800 border border-ink-700 text-white text-sm px-3 py-1.5">
                                </div>
                                <div class="col-span-4">
                                    <input type="text" name="results[{{ $index }}][value]" value="{{ $result['value'] ?? '' }}" placeholder="Value (e.g. +42%)" class="w-full rounded-lg bg-ink-800 border border-ink-700 text-white text-sm px-3 py-1.5">
                                </div>
                    ...
                    @else
                        ...
                        <div class="grid grid-cols-12 gap-2 result-row">
                            <div class="col-span-7">
                                <input type="text" name="results[0][metric]" placeholder="Metric Label (e.g. Load Time Reduction)" class="w-full rounded-lg bg-ink-800 border border-ink-700 text-white text-sm px-3 py-1.5">
                            </div>
                            <div class="col-span-4">
                                <input type="text" name="results[0][value]" placeholder="Value (e.g. 60%)" class="w-full rounded-lg bg-ink-800 border border-ink-700 text-white text-sm px-3 py-1.5">
                            </div>
                            <div class="col-span-1 flex items-center justify-center">
                                <button type="button" onclick="this.closest('.result-row').remove()" class="text-red-400 hover:text-red-300 text-sm">&times;</button>
                            </div>
            ...
            <div>
                <label class="block text-sm text-ink-300 mb-1">Featured Case Study Image</label>
    ...
    <script>
        document.getElementById('add-result-row').addEventListener('click', function() {
            ...
            newRow.innerHTML = `
                <div class="col-span-7">
                    <input type="text" name="results[${rowCount}][metric]" placeholder="Metric Label" class="w-full rounded-lg bg-ink-800 border border-ink-700 text-white text-sm px-3 py-1.5">
                </div>
                <div class="col-span-4">
                    <input type="text" name="results[${rowCount}][value]" placeholder="Value" class="w-full rounded-lg bg-ink-800 border border-ink-700 text-white text-sm px-3 py-1.5">
                </div>
                <div class="col-span-1 flex items-center justify-center">
                    <button type="button" onclick="this.closest('.result-row').remove()" class="text-red-400 hover:text-red-300 text-sm">&times;</button>
                </div>
